...
def build_nrm(meta: dict) -> Path:
    ...
    report = patch_nrm(ROOT / 'runtime/Russian_Language_Mod_template.nrm', ROOT / 'runtime/runtime_blob_manifest.json', ROOT / 'data/translation_workspace.csv', out, metadata_path=ROOT / 'metadata.json', thumb_path=ROOT / 'thumb.png')
...
def build_package(meta: dict, nrm_path: Path) -> Path:
    ...
    package_manifest = {'name': meta['name'], 'version_number': meta['version'], 'website_url': meta.get('website_url', ''), 'description': meta['description'], 'dependencies': meta.get('dependencies', [])}
